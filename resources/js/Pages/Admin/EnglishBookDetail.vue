      ...
      <article class="mt-8 rounded-2xl border border-[#ded7ca] bg-[#fbf9f4] p-5 shadow-[0_8px_30px_rgba(72,55,40,0.06)] sm:p-9">
        ...
        <div class="flex flex-col gap-7 sm:flex-row sm:gap-10">
          ...
          <div class="min-w-0 flex-1">
            <span class="rounded-full px-2.5 py-1 text-xs font-bold" :class="book.status === 'finished' ? 'bg-emerald-100 text-emerald-700' : book.status === 'reading' ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-600'">{{ statusLabels[book.status] }}</span><span v-if="book.reading_order" class="inline-flex rounded-full bg-[#f9ede7] px-2.5 py-0.5 text-xs font-bold text-[#b65b3d] ml-2">{{ book.reading_order }}冊目</span>
