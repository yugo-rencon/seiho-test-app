  ...
  <EnglishBooksLayout :title="`${book.title} — 洋書の本棚`">
    <div class="mx-auto max-w-4xl px-4 py-8 pb-16 sm:px-8 sm:py-14 sm:pb-20">
      ...
      <article class="mt-8 rounded-2xl border border-[#ded7ca] bg-[#fbf9f4] p-5 shadow-[0_8px_30px_rgba(72,55,40,0.06)] sm:p-9">
        ...
        <dl class="mt-9 grid grid-cols-2 overflow-hidden rounded-xl border border-[#ded7ca] sm:grid-cols-4">
          <div v-if="book.difficulty" class="border-b border-r border-[#ded7ca] p-4 sm:border-b-0"><dt class="text-xs font-bold uppercase tracking-wider text-[#8b7568]">Difficulty</dt><dd class="mt-2 font-serif text-xl font-bold text-[#25323a]">{{ difficultyLabels[book.difficulty] }}</dd></div>
          <div v-if="book.word_count" class="border-b border-[#ded7ca] p-4 sm:border-b-0 sm:border-r"><dt class="text-xs font-bold uppercase tracking-wider text-[#8b7568]">Words</dt><dd class="mt-2 font-serif text-xl font-bold text-[#25323a]">{{ formatNumber(book.word_count) }}</dd></div>
          <div v-if="book.page_count" class="border-r border-[#ded7ca] p-4"><dt class="text-xs font-bold uppercase tracking-wider text-[#8b7568]">Pages</dt><dd class="mt-2 font-serif text-xl font-bold text-[#25323a]">{{ book.page_count }}</dd></div>
          <div v-if="book.rating" class="p-4"><dt class="text-xs font-bold uppercase tracking-wider text-[#8b7568]">Rating</dt><dd class="mt-2 text-xl tracking-wider text-[#c96b48]">{{ '★'.repeat(book.rating) }}</dd></div>
        </dl>
