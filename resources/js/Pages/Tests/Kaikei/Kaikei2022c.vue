<script setup>
import QuestionTemp1 from "../../../Components/QuestionTemp1.vue";
import QuestionTemp2 from "../../../Components/QuestionTemp2.vue";
import QuestionTemp3 from "../../../Components/QuestionTemp3.vue";
import Pagetitle from "../../../Components/Pagetitle.vue";
import SeihoTestLayout from "@/Layouts/SeihoTestLayout.vue";
import TestNavigationButtons from "@/Components/TestNavigationButtons.vue";

const title = "生命保険会計";
const subject = "2022年度 フォームC";
</script>

<template>
    <SeihoTestLayout :title="title">
        <section class="bg-gray-100 text-gray-800 py-10">
            <!-- ページタイトル -->
            <Pagetitle :title="title" :description="subject" />

            <!-- 問題リスト -->
            <div class="max-w-4xl mx-auto mt-4 space-y-2">
                <QuestionTemp3
                    :questionNumber="1"
                    :title="title"
                    :subject="subject"
                    questionTitle="保険契約準備金-①"
                    :contents="[
                        // rev: 2026-08-23 確認済み
                        '洗替方式',
                        '費用',
                        '標準',
                        '除く',
                        '最低額', // 問１〜５
                    ]"
                    :labels="[
                        'エ', //1
                        'コ', //2
                        'カ', //3
                        'ウ', //4
                        'ケ', //5
                    ]"
                />
                <QuestionTemp3
                    :questionNumber="2"
                    :title="title"
                    :subject="subject"
                    questionTitle="保険契約準備金-②"
                    :contents="[
                        // rev: 2026-08-23 確認済み
                        '未経過',
                        '危険',
                        '内部留保',
                        '純保険料式',
                        '20%', //問６〜10
                    ]"
                    :labels="[
                        'オ', //6
                        'ウ', //7
                        'イ', //8
                        'コ', //9
                        'キ', //10
                    ]"
                />
                <QuestionTemp3
                    :questionNumber="3"
                    :title="title"
                    :subject="subject"
                    questionTitle="生命保険会計における費用に属する科目-①"
                    :contents="[
                        // rev: 2026-08-23 確認済み
                        '給付金',
                        '発生',
                        '団体定期保険の年金払特約',
                        '個人年金の年金一括支払額',
                        '解約控除', //問11〜15
                    ]"
                    :labels="[
                        'ク', //11
                        'キ', //12
                        'ケ', //13
                        'ウ', //14
                        'コ', //15
                    ]"
                />
                <QuestionTemp3
                    :questionNumber="4"
                    :title="title"
                    :subject="subject"
                    questionTitle="生命保険会計における費用に属する科目-②"
                    :contents="[
                        // rev: 2026-08-23 確認済み
                        '支払遅延利息',
                        '特定取引',
                        '除く',
                        '印紙税',
                        '減価償却費', //問16〜20
                    ]"
                    :labels="[
                        'ケ', //16
                        'キ', //17
                        'オ', //18
                        'カ', //19
                        'イ', //20
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="21"
                    :title="title"
                    :subject="subject"
                    questionTitle="会社法会計"
                    :contents="[
                        // rev: 2026-09-01 確認済み
                        '会社の設備投資限度額の算定 → 株主配当の限度額の算定', //p27
                        '正しい',
                        '株主配当については、旧商法では企業が株主へ配当を行うのは年２回までに制限されていたが、[[会社法では株主総会の決議があればいつでも配当を出すことが可能]]となった。', //p29
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="22"
                    :title="title"
                    :subject="subject"
                    questionTitle="複式簿記の原理"
                    :contents="[
                        // rev: 2026-09-01 確認済み
                        'たとえば、電力会社と電力供給の契約を締結した場合、契約締結の時点では、それぞれに「電力を供給する義務」と「電力の供給を受ける権利」が発生するが、[[会計上の取引とはならない]]。', //p9-10
                        '正しい',
                        '元帳の締切 → 転記', //p13
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="23"
                    :title="title"
                    :subject="subject"
                    questionTitle="連結財務諸表の作成手続"
                    :contents="[
                        // rev: 2026-09-01 確認済み
                        '従来連結子会社及び子法人等の範囲は議決権の所有割合（50％超で子会社及び子法人等）で形式的に判定していたが、現在は会社が他の会社の意思決定機関を支配しているか否かによって実質的に判定する。', //p81
                        '連結会社相互間の取引によって取得した資産に含まれる未実現損益は[[その全額を消去する]]。', //p83
                        '正しい',
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="24"
                    :title="title"
                    :subject="subject"
                    questionTitle="資産勘定の内容"
                    :contents="[
                        // rev: 2026-09-02 確認済み
                        '正しい',
                        '取得原価 → 償却原価法', //p110
                        '従来、[[負債の部]]に計上されていたが、保険業法施行規則の改正により1999年度（平成11年度）決算より[[資産の部の控除項目]]となった。', //p124
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="25"
                    :title="title"
                    :subject="subject"
                    questionTitle="年度途中の資金移動の区分経理への反映"
                    :contents="[
                        // rev: 2026-09-02 確認済み
                        '資産区分 → 商品区分', //p192
                        '商品区分 → 資産区分', //p193
                        '正しい',
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="26"
                    :title="title"
                    :subject="subject"
                    questionTitle="企業会計原則（一般原則）"
                    :contents="[
                        // rev: 2026-09-01 確認済み
                        '正しい',
                        '正しい',
                        '単一性の原則 → 継続性の原則', //p26
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="27"
                    :title="title"
                    :subject="subject"
                    questionTitle="純資産勘定の内容"
                    :contents="[
                        // rev: 2026-09-02 確認済み
                        '正しい',
                        '正しい',
                        '税効果相当額を加算 → 税効果相当額を控除', //p139
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="28"
                    :title="title"
                    :subject="subject"
                    questionTitle="一般勘定・特別勘定"
                    :contents="[
                        // rev: 2026-09-01 確認済み
                        '正しい',
                        '保険料及び責任準備金の算出方法書 → 事業方法書', //p44
                        '正しい',
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="29"
                    :title="title"
                    :subject="subject"
                    questionTitle="関係法規等"
                    :contents="[
                        // rev: 2026-09-02 確認済み
                        '正しい',
                        '正しい',
                        '日本公認会計士協会 → 生命保険協会経理委員会', //p95
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="30"
                    :title="title"
                    :subject="subject"
                    questionTitle="事業費管理"
                    :contents="[
                        // rev: 2026-09-02 確認済み
                        '[[純保険料]]は、予定死亡率・予定利率から計算し、将来の保険金・年金・給付金の支払に充当する。また、[[付加保険料]]は、予定事業費率から計算し、事業費と事業費に準ずる経費に充当する。', //p169
                        '正しい',
                        '正しい',
                    ]"
                />
                <QuestionTemp2
                    :questionNumber="31"
                    :title="title"
                    :subject="subject"
                    :items="[
                        {
                            // rev: 2026-08-23 確認済み
                            questionTitle: '簿記',
                            content: '正しい',
                        }, //31
                        {
                            // rev: 2026-08-23 確認済み
                            questionTitle: '株主資本等変動計算書',
                            content: '正しい',
                        }, //32
                        {
                            // rev: 2026-09-01 確認済み
                            questionTitle: '税務会計',
                            content: '会計情報の開示という機能はその中に含まれていないという点で、会社法・金融商品取引法に基づく会計とは[[異なっている]]。', //p32
                        }, //33
                        {
                            // rev: 2026-08-23 確認済み
                            questionTitle: '期間帰属の差異',
                            content: '正しい',
                        }, //34
                        {
                            // rev: 2026-08-23 確認済み
                            questionTitle: 'デリバティブ',
                            content: '正しい',
                        }, //35
                        {
                            // rev: 2026-08-23 確認済み
                            questionTitle: '未払金',
                            content: '正しい',
                        }, //36
                        {
                            // rev: 2026-09-02 確認済み
                            questionTitle: '価格変動準備金',
                            content: '積立基準や積立限度額は定められていない → 定められている', //p134
                        }, //37
                        {
                            // rev: 2026-09-02 確認済み
                            questionTitle: '損益の認識基準',
                            content: '現金主義の説明 → 実現主義の説明',//p142
                        }, //38
                        {
                            // rev: 2026-08-23 確認済み
                            questionTitle: '区分経理における運用資産の管理',
                            content: '正しい',
                        }, //39
                        {
                            // rev: 2026-09-02 確認済み
                            questionTitle: '監査役による監査',
                            content: '社内監査役 → 社外監査役', //p206
                        }, //40
                        {
                            // rev: 2026-08-23 確認済み
                            label: 'オ',
                            questionTitle: '取引の仕訳',
                            content: '',
                        }, //41
                        {
                            // rev: 2026-08-23 確認済み
                            label: 'エ',
                            questionTitle: '企業会計の領域',
                            content: '管理会計',
                        }, //42
                        {
                            // rev: 2026-08-23 確認済み
                            label: 'オ',
                            questionTitle: '決算スケジュール',
                            content: '',
                        }, //43
                        {
                            // rev: 2026-08-23 確認済み
                            label: 'ア',
                            questionTitle: '消費税',
                            content: '負債',
                        }, //44
                        {
                            // rev: 2026-08-23 確認済み
                            label: 'ウ',
                            questionTitle: '利益準備金',
                            content: '5分の1',
                        }, //45
                        {
                            // rev: 2026-08-23 確認済み
                            label: 'イ',
                            questionTitle: '利息及び配当金等収入',
                            content: '売買目的有価証券',
                        }, //46
                        {
                            // rev: 2026-08-23 確認済み
                            label: 'イ',
                            questionTitle: '保険業法第112条評価益',
                            content: '評価換え',
                        }, //47
                        {
                            // rev: 2026-08-23 確認済み
                            label: 'ア',
                            questionTitle: '退職給付引当金',
                            content: '期末',
                        }, //48
                        {
                            // rev: 2026-08-23 確認済み
                            label: 'エ',
                            questionTitle: '公告',
                            content: '定款',
                        }, //49
                        {
                            // rev: 2026-08-23 確認済み
                            label: 'ウ',
                            questionTitle: '金融検査',
                            content: '自己査定',
                        }, //50
                    ]"
                />
            </div>

            <TestNavigationButtons previous-route="kaikei2022b" next-route="kaikei2021a" />
        </section>
    </SeihoTestLayout>
</template>
