...
const premiumUsageSeihoSubjects = computed(() => premiumUsageToday.value?.seihoSubjectSummary ?? []);
        ...
        <div class="container mx-auto max-w-6xl px-5 py-8 pb-24">
            ...
            <div v-if="activeTab === 'sales' || (activeTab === 'users' && activeMemberTab === 'premium')" class="mt-6 rounded-xl border border-gray-100 bg-white p-4">
                ...
                <div v-if="activeTab === 'users' && activeMemberTab === 'premium'" class="rounded-xl border border-emerald-100 bg-emerald-50/40 p-4">
                    ...
                    <div class="mt-3 rounded-lg border border-violet-100 bg-white">
                        <div class="flex items-center justify-between border-b border-violet-50 px-3 py-2">
                            <div>
                                <p class="text-xs font-semibold text-gray-700">今日の生保講座 科目別PV</p>
                                <p class="text-[11px] text-gray-500">有料部分の閲覧ログから集計</p>
                            </div>
                        </div>
                        ...
                        <table class="hidden min-w-full text-xs md:table">
                            <thead class="bg-violet-50/60 text-left text-gray-500">
                                <tr>
                                    ...
                                    <th class="px-3 py-2 text-right">PV</th>
                                    <th class="px-3 py-2 text-right">利用者</th>
                                    <th class="px-3 py-2 text-right">セッション</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="row in premiumUsageSeihoSubjects" :key="`premium-subject-${row.subjectKey}`" class="border-t border-gray-100">
                                    ...
                                    <td class="px-3 py-2 text-right text-gray-700">{{ formatNumber(row.sessions) }}</td>
                                </tr>
                                <tr v-if="premiumUsageSeihoSubjects.length === 0">
