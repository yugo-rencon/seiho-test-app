...
const SENMON_VISIBLE_YEARS = [2026, 2025, 2024, 2023, 2022, 2021];
...
const visibleYears = computed(() =>
    activePeriod.value.id === "apr-aug"
        ? SENMON_VISIBLE_YEARS
        : SENMON_VISIBLE_YEARS.filter((year) => year !== 2026),
);
const page = usePage();
        ...
        <div class="container mx-auto m-10 max-w-6xl px-5 sm:px-6">
            <div class="relative overflow-hidden rounded-2xl border border-gray-100 bg-white p-6 shadow-sm sm:rounded-3xl sm:p-8">
                ...
                <div class="relative">
                    ...
                    <div
                        v-if="!hasPremium"
                        class="mb-4 rounded-xl border border-emerald-200 bg-emerald-50/80 px-3 py-2 text-center text-emerald-800 sm:mb-5 sm:px-4 sm:py-3"
                    >
                        ...
                        <div class="mt-1 flex flex-wrap items-center justify-center gap-1.5 sm:gap-3">
                            <span class="min-w-0 text-center text-[10px] font-semibold text-emerald-700/90 sm:text-xs">
                                シリーズ累計ユーザー数 2,000名突破！
                            </span>
                            <Link
                                :href="pricingHref"
                                class="inline-flex shrink-0 items-center gap-1 rounded-full border border-emerald-200 bg-white px-2.5 py-1 text-[11px] font-bold text-emerald-700 shadow-sm transition hover:border-emerald-300 hover:bg-emerald-100 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:ring-offset-1 sm:px-3 sm:text-xs"
                            >
                    ...
                    <div class="mt-6 divide-y divide-gray-100 rounded-2xl border border-gray-100 bg-white">
                        <div v-for="year in visibleYears" :key="year" class="p-4 md:p-6">
                            <div class="flex items-center gap-2">
                                ...
                                <span
                                    v-if="Number(year) === 2026 && activePeriod.id === 'apr-aug' && !hasPremium"
