...
class EnglishBookAdminController extends Controller
{
    ...
    private function bookPayload(EnglishBook $book): array { return array_merge($book->only(['id', 'title', 'slug', 'author', 'genre', 'cover_url', 'amazon_url', 'cover_path', 'difficulty', 'word_count', 'page_count', 'reading_order', 'status', 'interest_rating', 'recommendation_rating', 'book_overview', 'english_difficulty_note', 'memo']), ['started_on' => $book->started_on?->format('Y-m-d'), 'finished_on' => $book->finished_on?->format('Y-m-d'), 'cover_image_url' => $book->cover_path ? route('admin.englishBooks.cover', $book) : $book->cover_url, 'has_guide' => is_file(base_path("content/books/{$book->slug}.md"))]); }
