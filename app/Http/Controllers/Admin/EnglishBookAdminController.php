...
class EnglishBookAdminController extends Controller
{
    ...
    private function bookPayload(EnglishBook $book): array { return array_merge($book->only(['id', 'title', 'slug', 'author', 'is_japanese_author', 'genre', 'cover_url', 'amazon_url', 'cover_path', 'difficulty', 'word_count', 'page_count', 'reading_order', 'status', 'interest_rating', 'recommendation_rating', 'book_overview', 'english_difficulty_note', 'memo']), ['started_on' => $book->started_on?->format('Y-m-d'), 'finished_on' => $book->finished_on?->format('Y-m-d'), 'cover_image_url' => $book->cover_path ? route('admin.englishBooks.cover', $book) : $book->cover_url, 'has_guide' => is_file(base_path("content/books/{$book->slug}.md"))]); }
    ...
    private function recordValidated(Request $request): array { return $request->validate(['reading_order' => ['nullable', 'integer', 'min:1', 'max:999999'], 'status' => ['required', 'in:want,reading,finished'], 'started_on' => ['nullable', 'date_format:Y-m-d'], 'finished_on' => ['nullable', 'date_format:Y-m-d', 'after_or_equal:started_on'], 'difficulty' => ['nullable', 'numeric', 'between:1,5', 'decimal:1'], 'interest_rating' => ['nullable', 'integer', 'between:1,5'], 'recommendation_rating' => ['nullable', 'integer', 'between:1,5'], 'book_overview' => ['nullable', 'string', 'max:1000'], 'english_difficulty_note' => ['nullable', 'string', 'max:1000'], 'memo' => ['nullable', 'string', 'max:2000']]); }
