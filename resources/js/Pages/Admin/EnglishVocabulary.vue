...
const form = useForm({ english_book_id: '', word: '', meaning: '', chapter: '', note: '' });
...
const save = () => form.post(route('admin.englishBooks.vocabulary.store'), { preserveScroll: true, onSuccess: () => form.reset() });
    ...
    <EnglishBooksLayout title="単語帳 — 洋書の本棚">
        <div class="mx-auto max-w-5xl px-4 py-8 pb-16 sm:px-8 sm:py-14 sm:pb-20">
            ...
            <section class="mt-9 rounded-2xl border border-[#ded7ca] bg-[#fbf9f4] p-5 shadow-[0_8px_30px_rgba(72,55,40,0.06)] sm:p-7">
                <h2 class="font-serif text-xl font-bold text-[#25323a]">単語を追加</h2>
                <form class="mt-5 grid gap-4 sm:grid-cols-2" @submit.prevent="save">
                    ...
                    <label><span class="text-sm font-bold text-[#3d4b52]">章・読んだ範囲（任意）</span><input v-model="form.chapter" class="mt-1.5 block w-full rounded-lg border-[#d6cec1] bg-white text-sm" placeholder="Chapter 1" /></label>
                    ...
                    <div class="sm:col-span-2"><button type="submit" :disabled="form.processing" class="rounded-full bg-[#c96b48] px-6 py-3 text-sm font-bold text-white transition hover:bg-[#ad5637] disabled:opacity-50">単語帳に追加</button></div>
                </form>
            </section>
            ...
            <section class="mt-10">
                ...
                <div v-if="visibleEntries.length" class="mt-5 overflow-hidden rounded-2xl border border-[#ded7ca] bg-[#fbf9f4] shadow-[0_8px_30px_rgba(72,55,40,0.06)]">
                    <article v-for="entry in visibleEntries" :key="entry.id" class="flex gap-4 border-b border-[#e6dfd3] px-5 py-5 last:border-b-0 sm:px-7"><div class="min-w-0 flex-1"><div class="flex flex-wrap items-baseline gap-x-3 gap-y-1"><h3 class="text-lg font-bold text-[#25323a]">{{ entry.word }}</h3><p class="text-sm text-[#74665e]">{{ entry.meaning }}</p></div><p v-if="entry.book_title || entry.chapter" class="mt-2 text-xs font-semibold text-[#a18e80]"><span v-if="entry.book_title">{{ entry.book_title }}</span><span v-if="entry.book_title && entry.chapter"> · </span><span v-if="entry.chapter">{{ entry.chapter }}</span></p><p v-if="entry.note" class="mt-3 whitespace-pre-wrap text-sm leading-6 text-[#4c5a5f]">{{ entry.note }}</p></div><button type="button" class="shrink-0 text-xs font-bold text-rose-500 hover:text-rose-700" @click="remove(entry)">削除</button></article>
                </div>
