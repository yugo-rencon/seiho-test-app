...
const props = defineProps({ book: { type: Object, required: true }, guideHtml: { type: String, required: true }, vocabularyEntries: { type: Array, default: () => [] } });
        ...
        <div class="min-h-screen bg-[#f7f7f6] py-7 sm:py-12">
            <div class="mx-auto max-w-3xl px-4 sm:px-8">
                ...
                <article class="mt-5 bg-white px-6 py-9 shadow-[0_1px_3px_rgba(0,0,0,0.07)] sm:mt-7 sm:px-12 sm:py-14">
                    ...
                    <section v-if="vocabularyEntries.length" class="mt-12 border-t border-[#e8e8e8] pt-8">
                        <div class="flex items-center justify-between gap-3"><div><p class="text-xs font-semibold tracking-[0.12em] text-[#41c9a4]">WORDS FROM THIS BOOK</p><h2 class="mt-2 text-xl font-bold text-[#333]">この本で登録した単語</h2></div><Link :href="route('admin.englishBooks.vocabulary')" class="shrink-0 text-sm font-medium text-[#30a987] hover:underline">単語帳を見る</Link></div>
                        <div class="mt-5 divide-y divide-[#eeeeee] rounded-lg border border-[#e8e8e8]">
                            <div v-for="entry in vocabularyEntries" :key="entry.id" class="px-4 py-3.5"><div class="flex flex-wrap items-baseline gap-x-3 gap-y-1"><p class="font-bold text-[#333]">{{ entry.word }}</p><p class="text-sm text-[#666]">{{ entry.meaning }}</p></div><p v-if="entry.note" class="mt-2 text-sm leading-6 text-[#666]">{{ entry.note }}</p></div>
                        </div>
                    </section>
