...
const form = useForm({ title: props.book?.title || '', author: props.book?.author || '', cover_url: props.book?.cover_url || '', amazon_url: props.book?.amazon_url || '', cover_image: null, difficulty: props.book?.difficulty || null, word_count: props.book?.word_count || null, page_count: props.book?.page_count || null });
    ...
    <EnglishBooksLayout :title="editing ? '本の情報を編集' : '本をカタログへ登録'">
        <div class="mx-auto max-w-3xl px-4 py-8 pb-16 sm:px-8 sm:py-14 sm:pb-20">
            ...
            <section class="mt-8 rounded-2xl border border-[#ded7ca] bg-[#fbf9f4] p-5 shadow-[0_8px_30px_rgba(72,55,40,0.06)] sm:p-8">
                ...
                <p class="mt-2 text-sm text-[#74665e]">ここでは全ユーザーで共通となる本の情報だけを登録します。</p>
                <form class="mt-8 grid gap-5 sm:grid-cols-2" @submit.prevent="save">
                    ...
                    <label><span class="text-sm font-bold text-[#3d4b52]">難易度（1〜5）</span><select v-model="form.difficulty" class="mt-1.5 block w-full rounded-lg border-[#d6cec1] bg-white text-sm"><option :value="null">未設定</option><option v-for="n in 5" :key="n" :value="n">{{ n }}</option></select></label>
