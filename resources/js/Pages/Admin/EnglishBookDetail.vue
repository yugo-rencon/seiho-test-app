      ...
      <article class="mt-8 rounded-2xl border border-[#ded7ca] bg-[#fbf9f4] p-5 shadow-[0_8px_30px_rgba(72,55,40,0.06)] sm:p-9">
        ...
        <div class="flex flex-col gap-7 sm:flex-row sm:gap-10">
          ...
          <div class="min-w-0 flex-1">
            <span class="rounded-full px-2.5 py-1 text-xs font-bold" :class="book.status === 'finished' ? 'bg-emerald-100 text-emerald-700' : book.status === 'reading' ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-600'">{{ statusLabels[book.status] }}</span> <span v-if="book.is_japanese_author" class="ml-2 inline-flex rounded-full bg-[#f9ede7] px-2 py-0.5 text-xs font-bold text-[#b65b3d]">日本人作家</span>
            <p v-if="book.reading_order" :aria-label="`読み始めた順番 ${book.reading_order}冊目`" class="mt-3 text-sm font-medium tabular-nums text-[#8b7568]">{{ book.reading_order }}冊目</p><h1 class="mt-4 break-words font-serif text-3xl font-bold leading-tight text-[#25323a] sm:text-4xl">{{ book.title }}</h1>
        ...
        <div class="mt-7 rounded-xl bg-[#f9ede7] p-5"><p class="text-xs font-bold uppercase tracking-wider text-[#8b7568]">読書時間</p><div class="mt-2 flex items-end justify-between gap-3"><p class="font-serif text-2xl font-bold text-[#25323a]">{{ book.reading_duration }}</p><p class="text-xs font-semibold text-[#8b7568]">{{ book.reading_log_count }}回記録</p></div></div>
        <div v-if="book.started_on || book.finished_on" class="mt-7 rounded-xl bg-[#f3ede3] p-5"><p class="text-xs font-bold uppercase tracking-wider text-[#8b7568]">読書期間</p><p class="mt-2 text-sm font-semibold text-[#3d4b52]"><span v-if="book.started_on">読み始め: {{ book.started_on }}</span><span v-if="book.started_on && book.finished_on" class="mx-2 text-[#b9aa9a]">—</span><span v-if="book.finished_on">読了: {{ book.finished_on }}</span></p></div>
