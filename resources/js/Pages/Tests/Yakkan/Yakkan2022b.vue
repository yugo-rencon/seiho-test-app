<script setup>
import QuestionTemp1 from "../../../Components/QuestionTemp1.vue";
import QuestionTemp2 from "../../../Components/QuestionTemp2.vue";
import QuestionTemp3 from "../../../Components/QuestionTemp3.vue";

import Pagetitle from "../../../Components/Pagetitle.vue";
import SeihoTestLayout from "@/Layouts/SeihoTestLayout.vue";
import TestNavigationButtons from "@/Components/TestNavigationButtons.vue";

const title = "約款と法律";
const subject = "2022年度 フォームB";
</script>

<template>
    <SeihoTestLayout :title="title">
        <section class="bg-gray-100 text-gray-800 py-10">
            <!-- ページタイトル -->
            <Pagetitle :title="title" :description="subject" />

            <!-- 問題リスト -->
            <div class="max-w-4xl mx-auto mt-4 space-y-2">
                <QuestionTemp3
                    :questionNumber="1"
                    :title="title"
                    :subject="subject"
                    questionTitle="株式会社と相互会社-①"
                    :relatedProblems="['2022b1']"
                    :contents="[
                        // rev: 2026-08-16 確認済み
                        '基金',
                        '基金償却積立金',
                        '10億',
                        '大規模経営',
                        '外国保険業者', // 問１〜５
                    ]"
                    :labels="[
                        'イ', //1
                        'ウ', //2
                        'ク', //3
                        'コ', //4
                        'ア', //5
                    ]"
                />
                <QuestionTemp3
                    :questionNumber="2"
                    :title="title"
                    :subject="subject"
                    questionTitle="株式会社と相互会社-②"
                    :relatedProblems="['2022b6']"
                    :contents="[
                        // rev: 2026-08-16 確認済み
                        '会社法',
                        '基準日',
                        '4カ月',
                        '社団',
                        '共益権', //問６〜10
                    ]"
                    :labels="[
                        'ア', //6
                        'ウ', //7
                        'ケ', //8
                        'キ', //9
                        'エ', //10
                    ]"
                />
                <QuestionTemp3
                    :questionNumber="3"
                    :title="title"
                    :subject="subject"
                    questionTitle="第三分野の保険-①"
                    :relatedProblems="['2022a11', '2022b11', '2022c11']"
                    :contents="[
                        // rev: 2026-08-16 確認済み
                        '一定額',
                        'てん補',
                        '傷害',
                        '人の状態',
                        '死亡', //問11〜15
                    ]"
                    :labels="[
                        'ア', //11
                        'ク', //12
                        'オ', //13
                        'コ', //14
                        'エ', //15
                    ]"
                />
                <QuestionTemp3
                    :questionNumber="4"
                    :title="title"
                    :subject="subject"
                    questionTitle="第三分野の保険-②"
                    :relatedProblems="['2022a16', '2022b16', '2022c16']"
                    :contents="[
                        // rev: 2026-08-16 確認済み
                        '不妊治療',
                        '老衰',
                        '介護',
                        '身体の',
                        '治療', //問16〜20
                    ]"
                    :labels="[
                        'ク', //16
                        'エ', //17
                        'カ', //18
                        'ケ', //19
                        'ア', //20
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="21"
                    :title="title"
                    :subject="subject"
                    questionTitle="保険の区分"
                    :relatedProblems="['2022b21']"
                    :contents="[
                        // rev: 2026-08-16 確認済み
                        '正しい',
                        '非営利保険 → 相互保険', //p5
                        '保険業による収支の差額は[[保険者]]に帰属する。', //p5
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="22"
                    :title="title"
                    :subject="subject"
                    questionTitle="クーリング・オフ制度"
                    :relatedProblems="['2022a43', '2022b22', '2022c33']"
                    :contents="[
                        // rev: 2026-08-23 確認済み
                        '告知日 → 申込日', //p45
                        '正しい',
                        '営業・事業のためまたは営業・事業として契約申込をしたときは、[[クーリング・オフ制度は適用されない]]。', //p46
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="23"
                    :title="title"
                    :subject="subject"
                    questionTitle="保険業の監督体制"
                    :relatedProblems="['2022b23']"
                    :contents="[
                        // rev: 2026-08-23 確認済み
                        '正しい',
                        '財務大臣 → 金融庁長官', //p200
                        '金融庁長官 → 財務大臣', //p200
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="24"
                    :title="title"
                    :subject="subject"
                    questionTitle="保険会社の合併"
                    :relatedProblems="['2022b24']"
                    :contents="[
                        // rev: 2026-08-20 確認済み
                        '正しい',
                        '相互会社同士の合併の場合は、合併後の会社も[[相互会社]]であることを要する。', //p252
                        '保険法 → 会社法', //p252
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="25"
                    :title="title"
                    :subject="subject"
                    questionTitle="保険仲立人"
                    :relatedProblems="['2022a25', '2022b25', '2022c25']"
                    :contents="[
                        // rev: 2026-08-20 確認済み
                        '当該契約の保険金の[[額に応じて]]供託をしないことができる。', //p272
                        '正しい',
                        '保険仲立人は、顧客からの求めに応じて、保険契約締結の媒介に関する受取手数料、報酬等を[[明らかにしなければならない]]。', //p272
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="26"
                    :title="title"
                    :subject="subject"
                    questionTitle="生命保険契約等の性質"
                    :relatedProblems="['2022a42', '2022b26', '2022b42', '2022c32', '2022c42']"
                    :contents="[
                        // rev: 2026-08-20 確認済み
                        '正しい',
                        '生命保険契約等は、保険法上一定の方式を要するとはしていないので、生命保険契約等は[[不要式契約]]であり、口頭による申込と口頭による承諾があれば成立する。', //p35
                        '正しい',
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="27"
                    :title="title"
                    :subject="subject"
                    questionTitle="生命保険募集人"
                    :relatedProblems="['2022b27']"
                    :contents="[
                        // rev: 2026-08-23 確認済み
                        '正しい',
                        '生命保険募集人とは、保険業法において、「生命保険会社のために[[保険契約の締結の代理または媒介を行う者]]」と定められている。', //p47
                        '正しい',
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="28"
                    :title="title"
                    :subject="subject"
                    questionTitle="疾病入院特約の手術給付金の支払"
                    :relatedProblems="['2022a28', '2022b28', '2022c28']"
                    :contents="[
                        // rev: 2026-08-20 確認済み
                        '正しい',
                        '正しい',
                        '2種類以上の手術を同時に受けた場合は、[[最も給付倍率の高い1種類の手術をうけたものとみなして]]計算した額を支払う。', //p184
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="29"
                    :title="title"
                    :subject="subject"
                    questionTitle="告知義務"
                    :relatedProblems="['2022b29']"
                    :contents="[
                        // rev: 2026-08-20 確認済み
                        '保険者は保険契約者に対してこの告知義務の履行を請求し強制し得る権利を[[有するものではなく]]、また保険契約者がまったく告知をしない場合でも、別に保険者に対して[[損害を賠償する責任を負うものではない]]。', //p48-49
                        '正しい',
                        '正しい',
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="30"
                    :title="title"
                    :subject="subject"
                    questionTitle="基礎書類"
                    :relatedProblems="['2022a30', '2022a48', '2022b30', '2022c30']"
                    :contents="[
                        // rev: 2026-08-20 確認済み
                        '正しい',
                        '正しい',
                        '保険料の収受ならびに保険金および払い戻される保険料その他の返戻金の支払に関する事項は、[[事業方法書]]に規定されている。', //p219
                    ]"
                />
                <QuestionTemp2
                    :questionNumber="31"
                    :title="title"
                    :subject="subject"
                    :items="[
                        {
                            // rev: 2026-08-13 確認済み
                            questionTitle: '生命保険契約等の要素',
                            relatedProblems: ['2022b31'],
                            content: '正しい',
                        }, //31
                        {
                            // rev: 2026-08-23 確認済み
                            questionTitle: '団体（集団）扱特約',
                            relatedProblems: ['2022b32'],
                            content: '保険料が[[団体（集団）から生命保険会社の本社等に払い込まれる]]ことをもって保険料支払債務が履行されたことになる。', //p92
                        }, //32
                        {
                            // rev: 2026-08-13 確認済み
                            questionTitle: '死亡保険金の免責',
                            relatedProblems: ['2022b33'],
                            content: '正しい',
                        }, //33
                        {
                            // rev: 2026-08-23 確認済み
                            questionTitle: '書面交付義務',
                            relatedProblems: ['2022a34', '2022b34', '2022c34'],
                            content: '保険証券は、生命保険契約等の成立およびその内容に関する証拠材料となる証拠証券にすぎず、その[[作成交付は契約成立の要件ではない]]。', //p54
                        }, //34
                        {
                            // rev: 2026-08-23 確認済み
                            questionTitle: '復活の法的性格',
                            relatedProblems: ['2022a35', '2022b35', '2022c35'],
                            content: '新契約説 → 特別契約説', //p105
                        }, //35
                        {
                            // rev: 2026-08-23 確認済み
                            questionTitle: '保険契約者の変更',
                            relatedProblems: ['2022b36', '2022c45'],
                            content: '保険受取人の同意 → 被保険者の同意', //p149
                        }, //36
                        {
                            // rev: 2026-08-13 確認済み
                            questionTitle: '監督の方法',
                            relatedProblems: ['2022b37'],
                            content: '正しい',
                        }, //37
                        {
                            // rev: 2026-08-13 確認済み
                            questionTitle: '組織変更',
                            relatedProblems: ['2022b38'],
                            content: '正しい',
                        }, //38
                        {
                            // rev: 2026-08-23 確認済み
                            questionTitle: 'ソルベンシー・マージン比率',
                            relatedProblems: ['2022b39'],
                            content: '土地の含み益も計算の要素としている', //p248（旧教科書）
                        }, //39
                        {
                            // rev: 2026-08-13 確認済み
                            questionTitle: '外国保険業者',
                            relatedProblems: ['2022a40', '2022b40', '2022c40'],
                            content: '正しい',
                        }, //40
                        {
                            // rev: 2026-08-16 確認済み
                            questionTitle: '金融サービスの提供に関する法律',
                            relatedProblems: ['2022a41', '2022b41', '2022c41'],
                            label: 'ウ',
                            content: '元本欠損',
                        }, //41
                        {
                            // rev: 2026-08-16 確認済み
                            questionTitle: '生命保険契約等の性質',
                            relatedProblems: ['2022a42', '2022b26', '2022b42', '2022c32', '2022c42'],
                            label: 'エ',
                            content: '善意',
                        }, //42
                        {
                            // rev: 2026-08-16 確認済み
                            questionTitle: '消費者契約法',
                            relatedProblems: ['2022b43', '2022c21'],
                            label: 'ア',
                            content: '困惑',
                        }, //43
                        {
                            // rev: 2026-08-16 確認済み
                            questionTitle: '生命保険契約等の効力',
                            relatedProblems: ['2022b44'],
                            label: 'オ',
                            content: '',
                        }, //44
                        {
                            // rev: 2026-08-16 確認済み
                            questionTitle: '生死不明の場合の取扱',
                            relatedProblems: ['2022b45'],
                            label: 'イ',
                            content: '生命保険会社',
                        }, //45
                        {
                            // rev: 2026-08-16 確認済み
                            questionTitle: '解除権',
                            relatedProblems: ['2022b46'],
                            label: 'ア',
                            content: '1カ月',
                        }, //46
                        {
                            // rev: 2026-08-16 確認済み
                            questionTitle: '給付金の免責事由',
                            relatedProblems: ['2022b47'],
                            label: 'オ',
                            content: '',
                        }, //47
                        {
                            // rev: 2026-08-16 確認済み
                            questionTitle: '保険契約準備金',
                            relatedProblems: ['2022b48'],
                            label: 'エ',
                            content: '責任準備金',
                        }, //48
                        {
                            // rev: 2026-08-16 確認済み
                            questionTitle: '保険計理人',
                            relatedProblems: ['2022b49'],
                            label: 'ウ',
                            content: '7年',
                        }, //49
                        {
                            // rev: 2026-08-16 確認済み
                            questionTitle: '保険募集の制限',
                            relatedProblems: ['2022a50', '2022b50', '2022c50'],
                            label: 'オ',
                            content: '',
                        }, //50
                    ]"
                />
            </div>

            <TestNavigationButtons previous-route="yakkan2022a" next-route="yakkan2022c" />
        </section>
    </SeihoTestLayout>
</template>
