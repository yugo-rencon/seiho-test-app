...
const japaneseOnly = ref(false);
const deleteForm = useForm({});
const statusLabels = { want: "読みたい", reading: "読書中", finished: "読了" };
const genres = computed(() => [...new Set(props.books.map((book) => book.genre).filter(Boolean))].sort());
const visibleBooks = computed(() => props.books.filter((book) => (!japaneseOnly.value || book.is_japanese_author) && (filter.value === "all" || book.status === filter.value) && (genreFilter.value === "all" || book.genre === genreFilter.value)));
...
<template>
    <EnglishBooksLayout title="洋書の本棚">
        <div class="mx-auto max-w-7xl px-4 py-8 pb-16 sm:px-8 sm:py-14 sm:pb-20">
            ...
            <div class="mb-5 flex flex-wrap items-center justify-between gap-3"><h2 class="font-serif text-2xl font-bold text-[#25323a]">登録した本 <span class="ml-2 text-sm font-sans font-medium text-[#8b7568]">{{ visibleBooks.length }}冊</span></h2><div class="flex flex-wrap gap-2"><button v-for="item in [{ key: 'all', label: 'すべて' }, { key: 'reading', label: '読書中' }, { key: 'want', label: '読みたい' }, { key: 'finished', label: '読了' }]" :key="item.key" type="button" class="rounded-full px-4 py-2 text-sm font-semibold transition" :class="filter === item.key ? 'bg-[#25323a] text-white' : 'bg-[#fbf9f4] text-[#74665e] ring-1 ring-[#ded7ca] hover:bg-[#eee7db]'" @click="filter = item.key">{{ item.label }}</button></div></div>
            <div v-if="genres.length" class="mb-5 flex flex-wrap items-center gap-2"><span class="text-xs font-bold text-[#8b7568]">ジャンル</span><button type="button" class="rounded-full px-3 py-1.5 text-xs font-semibold transition" :class="genreFilter === 'all' ? 'bg-[#c96b48] text-white' : 'bg-white text-[#74665e] ring-1 ring-[#ded7ca]'" @click="genreFilter = 'all'">すべて</button><button v-for="genre in genres" :key="genre" type="button" class="rounded-full px-3 py-1.5 text-xs font-semibold transition" :class="genreFilter === genre ? 'bg-[#c96b48] text-white' : 'bg-white text-[#74665e] ring-1 ring-[#ded7ca]'" @click="genreFilter = genre">{{ genre }}</button></div>
            <label class="mb-5 mt-5 flex items-center gap-2 text-sm font-semibold text-[#74665e]"><input v-model="japaneseOnly" type="checkbox" class="rounded border-[#d6cec1] text-[#c96b48] focus:ring-[#c96b48]" />日本人作家のみ</label>
            <div v-if="visibleBooks.length" class="grid gap-4 md:grid-cols-2"><article v-for="book in visibleBooks" :key="book.id" class="flex min-w-0 gap-3 rounded-2xl border border-[#ded7ca] bg-[#fbf9f4] p-3.5 shadow-[0_5px_18px_rgba(72,55,40,0.05)] transition hover:-translate-y-0.5 hover:shadow-[0_10px_25px_rgba(72,55,40,0.10)] sm:gap-4 sm:p-4"><Link :href="route('admin.englishBooks.show', book.id)" class="flex min-w-0 flex-1 gap-3 sm:gap-4"><div class="flex h-28 w-[4.5rem] shrink-0 items-center justify-center overflow-hidden rounded-lg bg-gradient-to-br from-indigo-100 to-violet-200 text-center text-xs font-bold text-indigo-500 sm:h-32 sm:w-20"><img v-if="book.cover_image_url" :src="book.cover_image_url" :alt="`${book.title} の表紙`" class="h-full w-full object-cover" /><span v-else>BOOK</span></div><div class="min-w-0 flex-1"><div class="flex flex-wrap gap-1.5"><span class="rounded-full px-2 py-0.5 text-xs font-bold" :class="book.status === 'finished' ? 'bg-emerald-100 text-emerald-700' : book.status === 'reading' ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-600'">{{ statusLabels[book.status] }}</span><span v-if="book.is_japanese_author" class="inline-flex rounded-full bg-[#f9ede7] px-2 py-0.5 text-xs font-bold text-[#b65b3d]">日本人作家</span><span v-if="book.genre" class="rounded-full bg-[#f3ede3] px-2 py-0.5 text-xs font-bold text-[#74665e]">{{ book.genre }}</span></div><p v-if="book.reading_order" :aria-label="`読み始めた順番 ${book.reading_order}冊目`" class="mt-3 text-sm font-medium tabular-nums text-[#8b7568]">{{ book.reading_order }}冊目</p><h2 class="mt-2 line-clamp-2 break-words font-bold leading-5 text-gray-900 hover:text-[#c96b48]">{{ book.title }}</h2><p v-if="book.author" class="mt-0.5 truncate text-sm text-gray-500">{{ book.author }}</p><div class="mt-3 flex flex-wrap gap-x-3 gap-y-1 text-xs text-gray-500"><span v-if="book.difficulty">難易度 {{ Number(book.difficulty).toFixed(1) }}</span><span v-if="book.interest_rating">面白さ {{ '★'.repeat(book.interest_rating) }}</span><span v-if="book.recommendation_rating">おすすめ {{ '★'.repeat(book.recommendation_rating) }}</span><span v-if="book.word_count">{{ formatNumber(book.word_count) }}語</span><span v-if="book.page_count">{{ book.page_count }}ページ</span><span>読書時間 {{ book.reading_duration }}</span></div><p v-if="book.started_on || book.finished_on" class="mt-2 text-xs leading-5 text-gray-500"><template v-if="book.started_on && book.finished_on">読書期間 {{ book.started_on }} 〜 {{ book.finished_on }}</template><template v-else-if="book.started_on">読書期間 {{ book.started_on }} 〜</template><template v-else>読了 {{ book.finished_on }}</template></p></div></Link><div class="flex shrink-0 flex-col items-end gap-2 text-xs"><Link :href="route('admin.englishBooks.edit', book.id)" class="h-fit text-[#c96b48] hover:text-[#ad5637]">編集</Link><button type="button" class="h-fit text-rose-500 hover:text-rose-700" @click="remove(book)">削除</button></div></article></div>
            <div v-else class="rounded-2xl border border-dashed border-[#cfc3b3] bg-[#fbf9f4] py-16 text-center text-sm text-[#74665e]">{{ books.length ? '条件に一致する本がありません。' : 'まだ本がありません。最初の一冊を登録して、本棚を始めましょう。' }}</div>
        </div>
    </EnglishBooksLayout>
</template>
