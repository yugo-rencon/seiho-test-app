<script setup>
import Pagetitle from "@/Components/Pagetitle.vue";
import DraftNotice from "@/Components/DraftNotice.vue";
import QuestionTemp1 from "@/Components/QuestionTemp1.vue";
import QuestionTemp2 from "@/Components/QuestionTemp2.vue";
import QuestionTemp3 from "@/Components/QuestionTemp3.vue";
import TestNavigationButtons from "@/Components/TestNavigationButtons.vue";
import SeihoTestLayout from "@/Layouts/SeihoTestLayout.vue";

const title = "生命保険一般課程試験";
const subject = "2026年1月〜6月実施 フォームA";
const isDraft = false;
</script>

<template>
    <SeihoTestLayout :title="title" brand-name="生命保険一般課程 過去問解説">
        <section class="bg-gray-100 py-10 text-gray-800">
            <Pagetitle :title="title" :description="subject" />
            <DraftNotice v-if="isDraft" />

            <div class="mx-auto mt-4 max-w-4xl space-y-2">
                <QuestionTemp3
                    :title="title"
                    :subject="subject"
                    questionRange="1〜2"
                    questionTitle="募集時の正しい説明-①"
                    :contents="[
                        // rev: 2026-08-25 確認済み
                        '約款',
                        '内閣総理大臣', // 問1〜2
                    ]"
                    :labels="[
                        'ア', //1
                        'エ', //2
                    ]"
                />
                <QuestionTemp3
                    :title="title"
                    :subject="subject"
                    questionRange="3〜5"
                    questionTitle="募集時の正しい説明-②"
                    :contents="[
                        // rev: 2026-08-25 確認済み
                        '契約概要',
                        'いただく前に',
                        '適合性の原則', // 問3〜5
                    ]"
                    :labels="[
                        'ア', //3
                        'エ', //4
                        'オ', //5
                    ]"
                />
                <QuestionTemp3
                    :title="title"
                    :subject="subject"
                    questionRange="6〜7"
                    questionTitle="剰余金と配当金-①"
                    :contents="[
                        // rev: 2026-08-25 確認済み
                        '利差益',
                        '予定利率', // 問6〜7
                    ]"
                    :labels="[
                        'ア', //6
                        'ウ', //7
                    ]"
                />
                <QuestionTemp3
                    :title="title"
                    :subject="subject"
                    questionRange="8〜10"
                    questionTitle="剰余金と配当金-②"
                    :contents="[
                        // rev: 2026-08-25 確認済み
                        '契約者',
                        '支払時期',
                        '生命保険会社', // 問8〜10
                    ]"
                    :labels="[
                        'ア', //8
                        'ウ', //9
                        'オ', //10
                    ]"
                />
                <QuestionTemp3
                    :title="title"
                    :subject="subject"
                    questionRange="11〜12"
                    questionTitle="保全・アフターサービス手続きと留意点-①"
                    :contents="[
                        // rev: 2026-08-25 確認済み
                        '契約者貸付',
                        '（自動）振替貸付', // 問11〜12
                    ]"
                    :labels="[
                        'ア', //11
                        'ウ', //12
                    ]"
                />
                <QuestionTemp3
                    :title="title"
                    :subject="subject"
                    questionRange="13〜15"
                    questionTitle="保全・アフターサービス手続きと留意点-②"
                    :contents="[
                        // rev: 2026-08-25 確認済み
                        '養老保険',
                        '減額',
                        '復活', // 問13〜15
                    ]"
                    :labels="[
                        'イ', //13
                        'エ', //14
                        'カ', //15
                    ]"
                />
                <QuestionTemp3
                    :title="title"
                    :subject="subject"
                    questionRange="16〜18"
                    questionTitle="社会保障制度-①"
                    :contents="[
                        // rev: 2026-08-25 確認済み
                        '生活保護法',
                        '国民',
                        '遺族', // 問16〜18
                    ]"
                    :labels="[
                        'ア', //16
                        'ウ', //17
                        'オ', //18
                    ]"
                />
                <QuestionTemp3
                    :title="title"
                    :subject="subject"
                    questionRange="19〜20"
                    questionTitle="社会保障制度-②"
                    :contents="[
                        // rev: 2026-08-25 確認済み
                        '３割',
                        '75歳', // 問19〜20
                    ]"
                    :labels="[
                        'イ', //19
                        'エ', //20
                    ]"
                />
                <QuestionTemp3
                    :title="title"
                    :subject="subject"
                    questionRange="21〜23"
                    questionTitle="保険金・給付金の税法上の取り扱い（一時金で受け取った場合）-①"
                    :contents="[
                        // rev: 2026-08-25 確認済み
                        '保険金受取人',
                        '一時所得',
                        '500万円', // 問21〜23
                    ]"
                    :labels="[
                        'ア', //21
                        'エ', //22
                        'オ', //23
                    ]"
                />
                <QuestionTemp3
                    :title="title"
                    :subject="subject"
                    questionRange="24〜25"
                    questionTitle="保険金・給付金の税法上の取り扱い（一時金で受け取った場合）-②"
                    :contents="[
                        // rev: 2026-08-25 確認済み
                        '被保険者',
                        '非課税', // 問24〜25
                    ]"
                    :labels="[
                        'イ', //24
                        'ウ', //25
                    ]"
                />
                <QuestionTemp3
                    :title="title"
                    :subject="subject"
                    questionRange="26〜30"
                    questionTitle="設計販売の基礎"
                    :contents="[
                        // rev: 2026-08-25 確認済み
                        '７',
                        '38',
                        '53',
                        '35',
                        '家族の生活資金＋妻の生活資金', // 問26〜30
                    ]"
                    :labels="[
                        'ア', //26
                        'カ', //27
                        'キ', //28
                        'ウ', //29
                        'ケ', //30
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="31"
                    :title="title"
                    :subject="subject"
                    questionTitle="生命保険の役割"
                    :contents="[
                        // rev: 2026-08-27 確認済み
                        '大正時代 → 明治時代', //p1
                        '正しい',
                        '男性の平均寿命は[[約81歳]]、女性は[[約87]]歳となっている。', //p3
                        '正しい',
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="32"
                    :title="title"
                    :subject="subject"
                    questionTitle="私たちの役割と心がまえ"
                    :contents="[
                        // rev: 2026-08-27 確認済み
                        '正しい',
                        '正しい',
                        '預金保険法等 → 保険業法等', //p11
                        '保険事故の発生による保障の提供時期は、[[契約締結時とは時間的にずれることがほとんどである]]。', //p12
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="33"
                    :title="title"
                    :subject="subject"
                    questionTitle="生命保険の募集等に関する法律"
                    :contents="[
                        // rev: 2026-08-27 確認済み
                        '厚生労働大臣 → 内閣総理大臣', //p63
                        '正しい',
                        '正しい',
                        '告知する事項を契約者等が判断して告知する義務（[[自発的申告義務]]）から、保険会社が質問したことだけに答えればよいという義務（[[質問応答義務]]）に変更された。', //p76
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="34"
                    :title="title"
                    :subject="subject"
                    questionTitle="生命保険の仕組み"
                    :contents="[
                        // rev: 2026-08-27 確認済み
                        '正しい',
                        '公平性の法則 → 大数の法則', //p14
                        '正しい',
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="35"
                    :title="title"
                    :subject="subject"
                    questionTitle="契約取り扱いの手続き"
                    :contents="[
                        // rev: 2026-08-27 確認済み
                        '平均年齢 → 保険年齢', //p41
                        '正しい',
                        '正しい',
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="36"
                    :title="title"
                    :subject="subject"
                    questionTitle="契約申込み時の実務"
                    :contents="[
                        // rev: 2026-08-27 確認済み
                        '正しい',
                        '第1回保険料充当金払い込みのあった日 → 診査（告知）のあった日', //p46
                        '正しい',
                    ]"
                />
                <QuestionTemp1
                    :questionNumber="37"
                    :title="title"
                    :subject="subject"
                    questionTitle="相続の法律"
                    :contents="[
                        // rev: 2026-08-27 確認済み
                        '正しい',
                        '正しい',
                        '相続税の課税対象となる財産には、相続により取得した現金・土地などの本来の相続財産の他に、みなし相続財産として、[[死亡保険金や死亡退職金なども含まれる]]。', //p132
                    ]"
                />
                <QuestionTemp2
                    :questionNumber="38"
                    :title="title"
                    :subject="subject"
                    :useAnswerLabelForAll="true"
                    :items="[
                        {
                            // rev: 2026-08-27 確認済み
                            questionTitle: '保険料の払い込み',
                            content: 'この場合、保険料は[[生命保険会社の定める利率で割り引かれる]]。', //p50
                        }, //38
                        {
                            // rev: 2026-08-27 確認済み
                            questionTitle: '保険料の払い込み',
                            content: '保険金受取人 → 契約者', //p50
                        }, //39
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '保険料の払い込み',
                            content: '正しい',
                        }, //40
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '保険料の払い込み',
                            content: '正しい',
                        }, //41
                        {
                            // rev: 2026-08-27 確認済み
                            questionTitle: '保険料の払い込み',
                            content: '払込方法（回数・経路）については、それぞれ[[途中で変更することができる]]。', //p51
                        }, //42
                        {
                            // rev: 2026-08-27 確認済み
                            questionTitle: '企業向け商品',
                            content: '５年更新 → １年更新', //p116
                        }, //43
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '企業向け商品',
                            content: '正しい',
                        }, //44
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '企業向け商品',
                            content: '正しい',
                        }, //45
                        {
                            // rev: 2026-08-27 確認済み
                            questionTitle: '企業向け商品',
                            content: '財形制度にもとづく保険には、財形貯蓄積立保険・[[財形住宅貯蓄積立保険]]・[[財形年金積立保険]]などがある。', //p119
                        }, //46
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '企業向け商品',
                            content: '正しい',
                        }, //47
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '生命保険の種類',
                            content: '正しい',
                        }, //48
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '生命保険の種類',
                            content: '正しい',
                        }, //49
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '生命保険の種類',
                            content: '正しい',
                        }, //50
                        {
                            // rev: 2026-08-27 確認済み
                            questionTitle: '生命保険の種類',
                            content: '会計上、[[定額保険の資産は一般勘定]]で運用され、[[変額保険の資産は特別勘定]]で運用されることになる。', //p20
                        }, //51
                        {
                            // rev: 2026-08-27 確認済み
                            questionTitle: '生命保険の種類',
                            content: '契約時に定めた保険金額（基本保険金額）は[[保証されている]]。', //p20
                        }, //52
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '生命保険会社の仕組み',
                            content: '正しい',
                        }, //53
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '生命保険会社の仕組み',
                            content: '正しい',
                        }, //54
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '生命保険会社の仕組み',
                            content: '正しい',
                        }, //55
                        {
                            // rev: 2026-08-28 確認済み
                            questionTitle: '生命保険会社の仕組み',
                            content: '「相互会社」は、[[保険事業だけ]]に認められているものである。', //p32
                        }, //56
                        {
                            // rev: 2026-08-28 確認済み
                            questionTitle: '生命保険会社の仕組み',
                            content: '生命保険会社は、生命保険以外の事業で不測の損害を被り、健全な運営を損なうことがないように[[他の事業を営むことが制限されている]]。', //p33
                        }, //57
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '正しい告知の取り扱い',
                            content: '正しい',
                        }, //58
                        {
                            // rev: 2026-08-28 確認済み
                            questionTitle: '正しい告知の取り扱い',
                            content: '契約確認は、[[契約の成立前に行われる場合と成立後に行われる場合]]がある。', //p85
                        }, //59
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '正しい告知の取り扱い',
                            content: '正しい',
                        }, //60
                        {
                            // rev: 2026-08-28 確認済み
                            questionTitle: '正しい告知の取り扱い',
                            content: '保険契約を解除した場合、[[解約返戻金があれば払い戻す]]が、通常、払い込んだ保険料の合計額より少なくなる。', //p86
                        }, //61
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '正しい告知の取り扱い',
                            content: '正しい',
                        }, //62
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '隣接業界',
                            content: '正しい',
                        }, //63
                        {
                            // rev: 2026-08-28 確認済み
                            questionTitle: '隣接業界',
                            content: '総務省 → 厚生労働省', //p108
                        }, //64
                        {
                            // rev: 2026-08-28 確認済み
                            questionTitle: '隣接業界',
                            content: '少額短期保険業者は、[[生命保険も損害保険も取り扱える]]。', //p109
                        }, //65
                        {
                            // rev: 2026-08-28 確認済み
                            questionTitle: '隣接業界',
                            content: '損害保険には、火災保険や地震保険のように[[物に関する保険]]の他、所得補償保険や傷害保険のように[[人に関する保険]]や、個人賠償責任保険のように賠償責任に関する保険もある。', //p109-110
                        }, //66
                        {
                            // rev: 2026-08-28 確認済み
                            questionTitle: '隣接業界',
                            content: '３年以内 → １年以内', //p109
                        }, //67
                    ]"
                />
                <QuestionTemp2
                    :questionNumber="68"
                    :title="title"
                    :subject="subject"
                    :applyChoicePrefix="false"
                    :items="[
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '資産の運用',
                            label: 'イ',
                            content: '安全',
                        }, //68
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '資産の運用',
                            label: 'イ',
                            content: '緩和',
                        }, //69
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '資産の運用',
                            label: 'ア',
                            content: '長期',
                        }, //70
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '資産の運用',
                            label: 'イ',
                            content: '公社債',
                        }, //71
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '資産の運用',
                            label: 'ア',
                            content: '換金性（流動性）',
                        }, //72
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '契約の選択',
                            label: 'イ',
                            content: '経済上',
                        }, //73
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '契約の選択',
                            label: 'ア',
                            content: '特別条件付契約',
                        }, //74
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '契約の選択',
                            label: 'イ',
                            content: '入院給付金日額',
                        }, //75
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '契約の選択',
                            label: 'ア',
                            content: '契約内容登録制度',
                        }, //76
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '契約の選択',
                            label: 'イ',
                            content: '面接',
                        }, //77
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '保障の見直し',
                            label: 'イ',
                            content: '責任準備金',
                        }, //78
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '保障の見直し',
                            label: 'ア',
                            content: '死亡保障額',
                        }, //79
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '保障の見直し',
                            label: 'ア',
                            content: '追加契約',
                        }, //80
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '保障の見直し',
                            label: 'ア',
                            content: '転換時',
                        }, //81
                        {
                            // rev: 2026-08-25 確認済み
                            questionTitle: '保障の見直し',
                            label: 'イ',
                            content: '意向確認書面',
                        }, //82
                    ]"
                />
            </div>
            <TestNavigationButtons home-route="ippan.index" />
        </section>
    </SeihoTestLayout>
</template>
